...
{
    /** 
    * Create SQL for running on plugin Activation and Deactivation
    * creates table for saving:
    * 1) Article SPS parameters and YC response
    * 2) Orders Yellowcube response
    ...
    */
    protected static $ycSql = array(
        'actSql' => array(
            'crtQuery' => array(
                // product
                "CREATE TABLE IF NOT EXISTS `asign_product` (
                  `id`           INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
                  `artid`        INT(10) UNSIGNED NOT NULL,
                  `lastSent`     TINYINT(1) NOT NULL,                  
                  `ycSpsDetails` TEXT NOT NULL,
                  `ycResponse`   TEXT NOT NULL,  
                  `ycReference`  INT(10) NOT NULL,                  
                  `createDate`   TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                  PRIMARY KEY (`id`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8",
                
                // orders
                "CREATE TABLE IF NOT EXISTS `asign_orders` (
                  `id`              INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
                  `ordid`           INT(10) UNSIGNED NOT NULL,
                  `lastSent`        TINYINT(1) NOT NULL,
                  `ycReference`     INT(10) NOT NULL,
                  `ycResponse`      TEXT NOT NULL,
                  `ycWabResponse`   TEXT NOT NULL,
                  `ycWarResponse`   TEXT NOT NULL,                  
                  PRIMARY KEY (`id`)
               ) ENGINE=InnoDB DEFAULT CHARSET=utf8",

                // inventory
                "CREATE TABLE IF NOT EXISTS `asign_yellowcube` (
                  `id`          INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
                  `ycarticlenr` VARCHAR(128) CHARACTER SET latin1 COLLATE latin1_general_ci NOT NULL,
                  `articlenr`   VARCHAR(128) CHARACTER SET latin1 COLLATE latin1_general_ci NOT NULL,
                  `artdesc`     VARCHAR(255) CHARACTER SET latin1 COLLATE latin1_general_ci NOT NULL,
    ...
    */
    ...
    /**
     * Triggers the plugin uninstallation
     *   
     * @return null
     */
    public function uninstall()
    {
        ...
        foreach ($this->aCronEntries as $action => $name) {
            Shopware()->Db()->query("delete from `s_crontab` where `action` = '" . $action . "'");  
        }
    ...
    public function onPostDispatchCheckout(Enlight_Event_EventArgs $arguments)
    {
        ...
        $view->extendsTemplate('frontend/plugins/asign_yellowcube/index.tpl');       
    }
    ...
    public function onPostDispatchRegister(Enlight_Event_EventArgs $arguments)
    {
        ...
        $view->extendsTemplate('frontend/plugins/asign_yellowcube/register_billing_fieldset.tpl');
    }
    ...
    public function onPostDispatchAccount(Enlight_Event_EventArgs $arguments)
    {
        ...
        $view->extendsTemplate('frontend/plugins/asign_yellowcube/account_billing_fieldset.tpl');
    }
    ...
    /**
     * Creates new tables as per Queries...
     *
     * @return null
     */
    public function ycubeCreateTables()
    {
        //run the query
        $myQueries = self::$ycSql["actSql"]["crtQuery"];
    ...
    public function ycubeAlterTable()
    {
        ...
        if (!$this->isColumnExistsInTable('s_articles', 'origin')) {
            Shopware()->Db()->query("ALTER TABLE `s_articles` ADD COLUMN `origin` INT(10) NOT NULL");
        }
        ...
        if (!$this->isColumnExistsInTable('s_order_details', 'origin')) {
            Shopware()->Db()->query("ALTER TABLE `s_order_details` ADD COLUMN `origin` INT(10) NOT NULL");
        }           
    }
    ...
    {
        //run the query
        $myQueries = self::$ycSql["actSql"]["snipQuery"];
        foreach ($myQueries as $query) {
            Shopware()->Db()->query($query);
        }        
    }
    ...
    {   
        $sResult = Shopware()->Db()->query("SHOW COLUMNS FROM `" . $sTable . "` LIKE '" . $sColumn . "'");
