...
class PdfaDocument extends Shopware_Components_Document
{
    ...
    public function pdfaRender($aRenderer = null)
	{
        ...
        $aHomeCountry = Shopware()->Db()->fetchRow("SELECT `id`, `countryname` FROM `s_core_countries` WHERE `countryiso` = 'CH'");
        Shopware()->Session()->shopLocaleId = $aHomeCountry['id'];

        $aShopData['country'] = $aHomeCountry['countryname'];
        Shopware()->Session()->shopData = $aShopData;

        // pluginpath assigned
        $pluginPath = 'http://' . $_SERVER['SERVER_NAME'];
        ...
        $data = $this->_template->fetch("documents/index_yc.tpl", $this->_view);
