...
{    
    ...
    {
        $this->objErrorLog = new Errorlogs();
    }
    ...
    public function autoSendYCOrders($sMode = null, $isCron = false)
    {
        ...
        try {
            ...
            if ($sMode === 'pp') {
                $sWhere = " and `paymentID` = 5 and `cleareddate` IS NOT NULL and status = 12"; // 12 - code for fully paid in s_core_states
            }
            ...
            if (count($aOrders) > 0) {
                foreach ($aOrders as $order) {
                    $oOrder = new Orders();                    
                    $ordid = $order['id'];
                    $oDetails = $oOrder->getOrderDetails($ordid, false, true);                    

                    // check if the Status in the Order table
                    $sRequestField = $this->_getOrderRequestField($ordid);
                    $iStatusCode = $this->getRecordedStatus($ordid, 'asign_orders', $sRequestField);
                    $sResponseType = '';
                    
                    // get YC response
                    $oYCube = new AsignYellowcubeCore();
                    if ($iStatusCode == null && $oOrder->getFieldData($ordid, $sRequestField) == '') {
                        // execute the order object
                        echo "Submitting Order for OrderID: " . $ordid . "\n";
                        $aResponse = $oYCube->createYCCustomerOrder($oDetails);                        
                    }  elseif ($iStatusCode < 100) {
                        // get the status
                        echo "Requesting WAB status for OrderID: " . $ordid . "\n";
                        $aResponse = $oYCube->getYCGeneralDataStatus($ordid, "WAB");
                        $sResponseType = 'WAB';
                    } elseif ($iStatusCode == 100) {
                        // get the WAR status
                        echo "Requesting WAR status for OrderID: " . $ordid . "\n";
                        $aResponse = $oYCube->getYCGeneralDataStatus($ordid, "WAR");
                        $sResponseType = 'WAR';
                    }                   

                    // increment the counter
                    if (isset($aResponse) && count((array)$aResponse) !== 0) {
                        $oOrder->saveResponseData($aResponse, $ordid);

                        /*if ($oOrder->isTrackingNrResponse()) {
        ...
        try {
            ...
            if (count($aArticles) > 0) {
                foreach ($aArticles as $article) {
                    $oArticle = new Product();
                    $artid = $article['id'];
                    $oDetails = $oArticle->getArticleDetails($article['id'], true);

                    $iStatusCode = $this->getRecordedStatus($ordid, 'asign_product');
                    ...
                    if ($iStatusCode != 10) {
                        ...
                        $aResponse = $oYCube->insertArticleMasterData($oDetails, $sFlag);
                    ...
                    // increment the counter
                    if (count($aResponse) > 0) {
                        $oArticle->saveResponseData($aResponse, $artid);
                        $iCount = $iCount + 1;
                    }
                }
            }
     ...
     * @return  string
     */
    protected function _getOrderRequestField($orderId)
    {
        $oOrder = new Orders();
        $sResponseField = '';

        if ($oOrder->getFieldData($orderId, self::YCRESPONSE) == '') {
            $sResponseField = self::YCRESPONSE;
        } elseif ($oOrder->getFieldData($orderId, self::YCWABRESPONSE) == '') {
            $sResponseField = self::YCRESPONSE;
        } elseif ($oOrder->getFieldData($orderId, self::YCWARRESPONSE) == '') {
