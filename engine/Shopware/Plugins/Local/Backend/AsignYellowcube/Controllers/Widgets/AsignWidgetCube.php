...
class Shopware_Controllers_Widgets_AsignWidgetCube extends Enlight_Controller_Action
{
    ...
    public function triggerYellowcubeAction()
	{
	    ...
	    $ordnr = $_SESSION['Shopware']['sOrderVariables']['sOrderNumber'];
	    ...
	    try{	        
            ...
            if ($aOrders['paymentid'] != "5") {
                ...
                if ($aResponse['success']) {
                    $oModel->saveResponseData($aResponse, $orderId, 'DC');
                } else {                    
                    echo '<div class="notice center bold">[Order: '.$ordnr.'] ' . $aResponse['message'] . '</div>'; 
                }
            }
        } catch(Exception $e) {
            echo '<div class="notice center bold">' . $e->getMessage() . '</div>';
        }
	}
